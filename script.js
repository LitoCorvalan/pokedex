...
function filterPokemon(searchTerm, typeFilters) {
    ...
    if (activeFormFilter) {
        ...
        filteredPokemon.forEach((pokemon) => {
            const speciesForms = bySpecies.get(parseInt(pokemon.number, 10));
            if (!speciesForms) return;
            speciesForms.forEach((form) => {
                // Los filtros de tipo se aplican sobre los tipos de la forma
                if (typeFilters.length > 0 &&
                    !typeFilters.every((t) => form.types.includes(t))) return;
...
function setupSearchSuggestions() {
    ...
    function selectAbility(key) {
        ...
        activeAbility = { name: entry.name, species: new Set(entry.species) };
        searchBox.value = "";
        closeSuggestions();
        refilter();
    }
    ...
    function renderAbilitySuggestions(term) {
        ...
        keys.forEach((key) => {
            ...
            item.innerHTML = `
                ...
                <span class="suggest-number">${entry.species.length} Pokémon</span>`;
            item.addEventListener("mousedown", (e) => {
                e.preventDefault();
                selectAbility(key);
            });
