...
function setupAbilitySearch() {
    const box = document.getElementById("abilitySearchBox");
    const suggestBox = document.getElementById("abilitySuggestions");
    if (!box || !suggestBox) return;
    ...
    function renderSuggestions(term) {
        ...
        if (!normalized) { closeSuggestions(); return; }
    ...
    box.addEventListener("input", () => {
        ...
        // Vaciar el campo quita el filtro de habilidad
        if (term === "" && activeAbility) {
            activeAbility = null;
            refilter();
        }
    ...
    box.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown" || e.key === "ArrowUp") {
            ...
            e.preventDefault();
            selectAbility(items[activeIndex].dataset.key);
        } else if (e.key === "Escape") {
            closeSuggestions();
        }
    });

    box.addEventListener("blur", () => setTimeout(closeSuggestions, 120));
}

function clearAllFilters() {
    document.getElementById("searchBox").value = "";
    const abilityBox = document.getElementById("abilitySearchBox");
    if (abilityBox) abilityBox.value = "";
    activeAbility = null;
    activeFormFilter = null;
    document.querySelectorAll("#formFilter .form-chip").forEach((c) => {
