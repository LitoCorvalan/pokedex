...
// Filtros de formas especiales y habilidad
let activeFormFilter = null;   // null | "mega" | "gmax"
let activeAbility = null;      // null | { name, species: Set<number> }
let searchMode = "pokemon";    // "pokemon" | "ability"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Filtros Mega / Gigamax (mutuamente excluyentes; re-click desactiva)
    document.querySelectorAll("#formFilter .form-chip").forEach((chip) => {
        chip.addEventListener("click", () => {
            activeFormFilter = activeFormFilter === chip.value ? null : chip.value;
            document.querySelectorAll("#formFilter .form-chip").forEach((c) => {
                updateButtonState(c, c.value === activeFormFilter);
                c.classList.toggle("active", c.value === activeFormFilter);
            });
            refilter();
        });
    });
    ...
    const filtersDone = document.getElementById("filtersDone");
    ...
    if (filtersDone) filtersDone.addEventListener("click", () => setFiltersOpen(false));
    if (filtersBackdrop) filtersBackdrop.addEventListener("click", () => setFiltersOpen(false));
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && filtersPanel.classList.contains("open")) setFiltersOpen(false);
    });
    ...
    searchBox.addEventListener("input", () => {
        if (searchMode === "pokemon") refilter();
    });

    // Selector de modo Pokémon / Habilidad
    document.querySelectorAll(".search-mode .mode-button").forEach((button) => {
        button.addEventListener("click", () => {
            if (searchMode === button.dataset.mode) return;
            searchMode = button.dataset.mode;
            document.querySelectorAll(".search-mode .mode-button").forEach((b) =>
                b.classList.toggle("active", b === button));
            searchBox.value = "";
            searchBox.placeholder = searchMode === "pokemon"
...
function refilter() {
    ...
    const searchTerm = searchMode === "pokemon" ? box.value.trim() : "";
    filterPokemon(searchTerm, getCurrentTypeFilters());
}
...
function filterPokemon(searchTerm, typeFilters) {
    ...
    if (typeFilters.length > 0 && !activeFormFilter) {
        filteredPokemon = filteredPokemon.filter((pokemon) => {
    ...
    if (activeAbility) {
        filteredPokemon = filteredPokemon.filter((pokemon) =>
            activeAbility.species.has(parseInt(pokemon.number, 10)));
    }
    ...
    const typeMatchesForm = (form) =>
        typeFilters.length === 0 || typeFilters.every((t) => form.types.includes(t));

    if (activeFormFilter) {
        // Solo especies con esa forma, mostrando la forma ya aplicada.
        // Una especie puede generar varias cards (Charizard: Mega X y Mega Y).
        const forms = activeFormFilter === "mega" ? MEGA_FORMS : GMAX_FORMS;
        ...
        filteredPokemon.forEach((pokemon) => {
            const speciesId = parseInt(pokemon.number, 10);
            const speciesForms = bySpecies.get(speciesId);
            if (!speciesForms) return;
            speciesForms.forEach((form) => {
                // Con habilidad activa, esa forma concreta debe tenerla
                if (activeAbility && !activeAbility.baseOwners.has(speciesId) &&
                    !(activeAbility.formOwners.get(speciesId) || []).includes(form.id)) return;
                if (!typeMatchesForm(form)) return;
                fragment.appendChild(createPokemonCard(pokemon, form));
                cardCount++;
            });
        });
    } else {
        filteredPokemon.forEach((pokemon) => {
            ...
            // Si solo formas especiales tienen la habilidad, mostrar ESAS formas
            // (evita creer que la forma base la posee)
            if (activeAbility && !activeAbility.baseOwners.has(speciesId)) {
                (activeAbility.formOwners.get(speciesId) || []).forEach((formId) => {
                    const info = FORM_INFO[formId];
                    if (!info) return;
                    const form = { id: formId, ...info };
                    if (!typeMatchesForm(form)) return;
                    fragment.appendChild(createPokemonCard(pokemon, form));
                    cardCount++;
                });
    ...
    updateResultsUI(cardCount, searchTerm, typeFilters);
}
...
function updateResultsUI(count, searchTerm, typeFilters) {
    ...
    // Chip de la habilidad aplicada (removible)
    const oldChip = document.getElementById("abilityChip");
    if (oldChip) oldChip.remove();
    if (activeAbility) {
        const chip = document.createElement("span");
        chip.id = "abilityChip";
        chip.className = "ability-chip";
        chip.innerHTML = `Habilidad: <strong>${activeAbility.name}</strong>`;
        const removeButton = document.createElement("button");
        removeButton.setAttribute("aria-label", `Quitar filtro de habilidad ${activeAbility.name}`);
        ...
        resultCount.after(chip);
    }
    if (clearButton) clearButton.hidden = !hasFilters;
    if (emptyState) emptyState.hidden = count !== 0;

    // Badge con la cantidad de filtros activos (boton mobile)
    const filtersCount = document.getElementById("filtersCount");
    if (filtersCount) {
        const active =
            typeFilters.length +
            genFilters.filter((g) => g.filterActive).length +
            (activeFormFilter ? 1 : 0) +
            (activeAbility ? 1 : 0);
        filtersCount.textContent = active;
        filtersCount.hidden = active === 0;
        document.getElementById("filtersToggle")
            ?.classList.toggle("has-filters", active > 0);
    }
}

// ---------- Autocompletado del buscador (Pokémon o Habilidad según el modo) ----------
function setupSearchSuggestions() {
    ...
    function selectAbility(key) {
        const entry = ABILITY_INDEX[key];
        // owners: [speciesId, 0] si la forma base la tiene;
        //         [speciesId, formId] si solo una forma especial la tiene.
        const species = new Set();
        const baseOwners = new Set();
        const formOwners = new Map(); // species -> [formIds]
        entry.owners.forEach(([sp, formId]) => {
            species.add(sp);
            if (formId === 0) {
                baseOwners.add(sp);
            } else {
                if (!formOwners.has(sp)) formOwners.set(sp, []);
                formOwners.get(sp).push(formId);
            }
        });
        activeAbility = { name: entry.name, species, baseOwners, formOwners };
        searchBox.value = "";
        closeSuggestions();
        refilter();
    }

    function renderPokemonSuggestions(term) {
        const matches = pokemonData.filter((p) => matchesSearch(p, term)).slice(0, 8);
        ...
        matches.forEach((p) => {
            ...
            item.innerHTML = `
                ...
                <span class="suggest-name"><strong>${displayName.slice(0, prefixLength)}</strong>${displayName.slice(prefixLength)}</span>
    ...
    function renderAbilitySuggestions(term) {
        const normalized = normalizeText(term);
        const keys = Object.keys(ABILITY_INDEX)
            .filter((k) => k.startsWith(normalized))
            .slice(0, 8);
        if (keys.length === 0) { closeSuggestions(); return; }
        ...
        keys.forEach((key) => {
            ...
            item.setAttribute("role", "option");
            item.innerHTML = `
                <span class="suggest-name"><strong>${entry.name.slice(0, term.length)}</strong>${entry.name.slice(term.length)}</span>
                <span class="suggest-number">${entry.owners.length} Pokémon</span>`;
            item.addEventListener("mousedown", (e) => {
                e.preventDefault();
                selectAbility(key);
            });
    ...
    searchBox.addEventListener("input", () => {
        ...
        if (term.length === 0) { closeSuggestions(); return; }
        if (searchMode === "pokemon") renderPokemonSuggestions(term);
        else renderAbilitySuggestions(term);
    });
    ...
    searchBox.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown" || e.key === "ArrowUp") {
            ...
            if (searchMode === "pokemon") selectPokemon(item);
            else selectAbility(item.dataset.key);
...
function createPokemonCard(pokemon, form = null) {
    const card = document.createElement("div");
    card.classList.add("card");

    const typeList = form ? form.types : pokemon.type;

    // Colores del tipo como variables CSS (una sola fuente de verdad: badges.js)
    const c1 = (TYPE_COLORS[typeList[0]] || {}).bg || "#A8A77A";
    const c2 = (TYPE_COLORS[typeList[1] || typeList[0]] || {}).bg || c1;
    ...
    name.textContent = form ? `${displayName} · ${form.form}` : displayName;
    ...
    if (form) {
        // Si la forma aun no tiene arte oficial, mostrar la forma base
        // (con spinner mientras carga el reemplazo)
        image.onerror = () => {
            image.onerror = null;
            image.src = pokemon.image;
            attachImageSpinner(image, imageButton);
        };
    }
    ...
    for (let i = 0; i < typeList.length; i++) {
        ...
        typeImage.src = getTypeImageURL(typeList[i]);
        typeImage.alt = `Tipo ${typeList[i]}`;
    ...
    detailButton.addEventListener("click", () => {
        openDetail(displayNumber, form ? form.id : null);
    });
