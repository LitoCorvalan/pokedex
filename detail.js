...
async function openDetail(dexNumber, targetPokemonId = null) {
    const modal = ensureModal();
    const body = modal.querySelector(".modal-body");
    modal.classList.add("open");
    document.body.style.overflow = "hidden";
    body.innerHTML = `<div class="modal-loading" role="status" aria-label="Cargando ficha">
        <div class="poke-spinner poke-spinner-lg"></div>
    </div>`;

    try {
        const species = await apiGet(`${API}/pokemon-species/${parseInt(dexNumber, 10)}`);
        const varieties = species.varieties;
        let formIndex = varieties.findIndex((v) => v.is_default);
        if (targetPokemonId !== null) {
            const target = varieties.findIndex((v) =>
                v.pokemon.url.match(/\/(\d+)\/?$/)?.[1] === String(targetPokemonId));
            if (target >= 0) formIndex = target;
        }
        if (formIndex < 0) formIndex = 0;
        await renderForm(body, species, varieties, formIndex);
    } catch (err) {
        ...
        body.querySelector(".retry-button").addEventListener("click", () => openDetail(dexNumber, targetPokemonId));
    }
}
...
async function renderForm(body, species, varieties, formIndex) {
    ...
    const artwork =
        ...
        `https://assets.pokemon.com/assets/cms2/img/pokedex/full/${String(species.id).padStart(3, "0")}.png`;
    ...
    body.innerHTML = `
        ...
        <div class="detail-columns">
            ...
            <section class="detail-section">
                <h3>Habilidades</h3>
                <ul class="abilities-list"><li class="section-loading" role="status" aria-label="Cargando habilidades"><span class="poke-spinner"></span></li></ul>
                <h3>Datos</h3>
                <p class="detail-meta">
        ...
        <section class="detail-section">
            ...
            <div class="evo-chain"><span class="section-loading" role="status" aria-label="Cargando línea evolutiva"><span class="poke-spinner"></span></span></div>
        </section>`;

    // Spinner mientras carga el arte del hero
    const heroArt = body.querySelector(".detail-art");
    const heroNav = body.querySelector(".hero-nav");
    if (heroArt && heroNav) attachImageSpinner(heroArt, heroNav);
